...
const withRheostat = (ChartCompo: any = null) =>
  React.memo((props: RheostatTypes) => {
    const {
    ...
    const [values, setValues] = useState(() =>
      inputValues.map((value) => new Animated.Value(value))
    );
    useEffect(() => {
      setHandlePos(
        inputValues.map(
    ...
    const [handleDimensions, setHandleDimensions] = useState({
      width: 0,
      height: 0,
    });
    ...
    const getClosestSnapPoint = (value: number, idx: number) => {
      ...
      if (handlePos[idx + 1] !== undefined) {
        ...
        validatedSnapPoints = snapPoints.filter(
          (snapP) => snapP < maxSnap - offsetSnap
        );
      } else if (handlePos[idx - 1] !== undefined) {
        const minSnap = (values[idx - 1] as any).__getValue();
        validatedSnapPoints = snapPoints.filter(
          (snapP) => snapP > minSnap + offsetSnap
        );
      }
    ...
    const moveSlide = (idx: number, gestureState: PanResponderGestureState) => {
      const { onValuesUpdated } = props;
      const { width } = containerSize;
      const proposedPosition =
        (gestureState.dx / width) * PERCENT_FULL + previousHandlePos[idx];
    ...
    const panResponders = useMemo(
      () =>
        handlePos.map((_val, idx) =>
          customPanResponder(idx, startSlide, moveSlide, endSlide)
        ),
    ...
    const pos = handlePos[0].interpolate({
      inputRange: [0, 100],
